...
corona_wide_all <- corona_wide_all %>% mutate(new_cases_cum = cumsum(new_cases))
...
maxKey <- plotData %>% filter(!is.na(value)) %>% group_by(key) %>% summarise(maxDate = max(dt)) %>% ungroup()
# weeksPast <- 3
weeksFuture <- 1.5

# the hospital case and deaths lag behind and need different windows at the moment.
# this needs to be adjusted later
baseWindow <- data.frame(key = c('new_cases', 'hospital_cases', 'deaths'), lag = c(2.5, 2.5, 2.5), stringsAsFactors = F)
...
predBase <- plotData %>% filter(dt >= startDate & !is.na(value))  %>% select(key, dt, value, lockdown, niceName, maxDate, startDate, predDate)

predBase <- predBase %>% mutate(value_log10 = log10(value))
...
lModel <- predBase %>% group_by (key) %>% 
  do(linearModel = tidy(lm(value_log10 ~ dt, data = .))) %>%
...
prediction <- prediction %>% mutate(value_log10 = intercept + as.numeric(dt) * gradient,
...
plotData <- add_lockdown_pred(plotData)
...
gp <- ggplot(plotData, aes(x= dt, y = rolling_avg, colour = lockdown)) + geom_point(aes(colour = lockdown)) + geom_line(aes(colour = lockdown)) + 
  ...
  geom_line(data = prediction, aes(x=dt, y=value, color=lockdown),  lwd = 0.75) + 
...
gp <- ggplot(plotData , aes(x= dt, y = rolling_avg, colour = lockdown)) + geom_point(aes(colour = lockdown)) + geom_line(aes(colour = lockdown)) +   # facet_wrap(~key, scales = 'free') + scale_y_log10() + xlab('Time') + ylab('7 day rolling average')  + 
  ...
  ggtitle(paste('UK Covid figures on ', max(plotData$dt), sep = '')) + theme_bw(base_size = 18) +   theme(axis.text.x=element_text(angle=60, hjust=1))
